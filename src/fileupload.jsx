...
export default function FileUploader() {
  ...
  function handleFilesAdded(fileList) {
    const newItems = Array.from(fileList).map(file => {
      ...
      if (file.size > MAX_FILE_SIZE_BYTES) {
        error = `File too large (${formatBytes(file.size)}). Max: ${formatBytes(MAX_FILE_SIZE_BYTES)}.`;
        status = 'error';
      } else if (ALLOWED_TYPES.length && !ALLOWED_TYPES.includes(file.type)) {
        error = `Invalid file type: ${file.type || 'unknown'}`;
        status = 'error';
      }

      return { id, file, preview, progress: 0, status, error, controller: null };
    });
  ...
  async function uploadSingle(item) {
    ...
    setFiles(prev =>
      prev.map(f =>
        f.id === item.id ? { ...f, status: 'uploading', controller, progress: 0, error: null } : f
      )
    );
    ...
    try {
      const res = await axios.post(`${UPLOAD_URL}/upload`, form, {
      ...
      setFiles(prev =>
        prev.map(f =>
          f.id === item.id ? { ...f, status: 'done', progress: 100, controller: null } : f
        )
      );
      return res.data;
    } catch (err) {
      const isAbort = axios.isCancel?.(err) || err.name === 'CanceledError';
      setFiles(prev =>
        prev.map(f =>
          f.id === item.id
            ? {
  ...
  function uploadAll() {
    files
      .filter(f => f.status === 'ready' || f.status === 'error' || f.status === 'cancelled')
      .forEach(f => uploadSingle(f));
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-500">
            Allowed: {ALLOWED_TYPES.join(', ')} • Max size: {formatBytes(MAX_FILE_SIZE_BYTES)}
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 space-y-3">
        {files.length === 0 && <p className="text-sm text-gray-500">No files added yet.</p>}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm">
                  ...
                  {f.status === 'uploading' && (
                    <span className="text-blue-600">Uploading {f.progress}%</span>
                  )}
